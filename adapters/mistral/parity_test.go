...
func TestParityFixtures(t *testing.T) {
	modelPath := os.Getenv("OMNITOKEN_MISTRAL_TEKKEN_JSON")
	fixturesPath := os.Getenv("OMNITOKEN_MISTRAL_PARITY_JSONL")
	if modelPath == "" || fixturesPath == "" {
		t.Skip("set OMNITOKEN_MISTRAL_TEKKEN_JSON and OMNITOKEN_MISTRAL_PARITY_JSONL")
	}
	engine, err := New(Options{
		Source:                  ModelSource{Path: modelPath},
		AllowUnsupportedPattern: os.Getenv("OMNITOKEN_MISTRAL_ALLOW_UNSUPPORTED_PATTERN") == "1",
	})
	if err != nil {
		t.Fatal(err)
	}
