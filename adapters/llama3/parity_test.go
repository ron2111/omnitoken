...
func TestParityFixtures(t *testing.T) {
	modelPath := os.Getenv("OMNITOKEN_LLAMA3_TOKENIZER_MODEL")
	fixturesPath := os.Getenv("OMNITOKEN_LLAMA3_PARITY_JSONL")
	if modelPath == "" || fixturesPath == "" {
		t.Skip("set OMNITOKEN_LLAMA3_TOKENIZER_MODEL and OMNITOKEN_LLAMA3_PARITY_JSONL")
	}
	engine, err := New(Options{Source: ModelSource{Path: modelPath}, Variant: parityVariant()})
	if err != nil {
		t.Fatal(err)
	}
...
func parityVariant() Variant {
	switch os.Getenv("OMNITOKEN_LLAMA3_VARIANT") {
	case string(VariantLlama31):
		return VariantLlama31
	case string(VariantLlama32):
		return VariantLlama32
	default:
		return VariantLlama3
	}
}
