...
func BenchmarkLlama3CountTokens(b *testing.B) {
...
func BenchmarkLlama3EncodeOrdinary(b *testing.B) {
	engine := benchmarkEngine(b)
	for name, text := range benchmarkTexts {
		b.Run(name, func(b *testing.B) {
			b.ReportAllocs()
...
func benchmarkEngine(b *testing.B) *Engine {
	b.Helper()
	path := os.Getenv("OMNITOKEN_LLAMA3_TOKENIZER_MODEL")
	if path == "" {
		b.Skip("set OMNITOKEN_LLAMA3_TOKENIZER_MODEL")
	}
