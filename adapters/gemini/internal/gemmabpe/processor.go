// Package gemmabpe provides a local Gemma/Gemini BPE tokenizer runtime.
//
// Portions are adapted from github.com/eliben/go-sentencepiece (Apache-2.0),
// with OmniToken-specific count-only and ID-only encode paths.
package gemmabpe
...
import (
	...
	"github.com/ron2111/omnitoken/adapters/gemini/internal/gemmabpe/model"
	"github.com/ron2111/omnitoken/adapters/gemini/internal/gemmabpe/prefixmatcher"
	"github.com/ron2111/omnitoken/adapters/gemini/internal/gemmabpe/priorityqueue"
...
type idSymListElem struct {
	prev, next int
	noMerge    bool
	symbol     string
}

type idMergeCandidate struct {
	left, right int
	length      int
	score       float32
}
...
func NewProcessor(protoReader io.Reader) (*Processor, error) {
	...
	proc.scratchPool.New = func() any {
		return &encodeScratch{buf: make([]byte, proc.maxPieceLength)}
	}
	return proc, nil
}
...
// EncodeIDs tokenizes text and returns token IDs without materializing Token.Text.
func (proc *Processor) EncodeIDs(text string) []int {
	ids := make([]int, 0, len(text)/4+1)
	proc.forEachID(text, func(id int) {
		ids = append(ids, id)
	})
	return ids
}

// CountTokens counts tokens without materializing the final token slice.
func (proc *Processor) CountTokens(text string) int {
	return proc.forEachID(text, nil)
}

func (proc *Processor) forEachID(text string, emit func(int)) int {
	scratch := proc.getScratch()
	defer proc.putScratch(scratch)

	text, scratch.normalized = normalizeInto(text, scratch.normalized)
	symList := scratch.syms[:0]
	if cap(symList) < len(text) {
		symList = make([]idSymListElem, 0, len(text))
	}
	for len(text) > 0 {
		slen, found := proc.symbolMatch(text)
		symList = append(symList, idSymListElem{
			noMerge: found,
			symbol:  text[:slen],
			prev:    len(symList) - 1,
			next:    len(symList) + 1,
	...
	mergeQueue := priorityqueue.NewWithBacking(scratch.queue, func(a, b idMergeCandidate) int {
		if a.score > b.score || (a.score == b.score && a.left < b.left) {
			return 1
		}
		return -1
	...
	buf := scratch.buf
	if cap(buf) < proc.maxPieceLength {
		buf = make([]byte, proc.maxPieceLength)
	}
	findMerged := func(x, y idSymListElem) (string, int, bool) {
		combinedLen := len(x.symbol) + len(y.symbol)
		if combinedLen > cap(buf) {
			return "", 0, false
		}
	...
	suggestNewMergePair := func(left, right int) {
		...
		if mergedSymbol, id, ok := findMerged(symList[left], symList[right]); ok {
			mergeQueue.Insert(idMergeCandidate{
				left:   left,
				right:  right,
				length: len(mergedSymbol),
				score:  proc.model.GetPieces()[id].GetScore(),
	...
	candidateIsDead := func(candidate idMergeCandidate) bool {
		leftSymbol := symList[candidate.left].symbol
		rightSymbol := symList[candidate.right].symbol
		return leftSymbol == "" || rightSymbol == "" || len(leftSymbol)+len(rightSymbol) != candidate.length
	}
	...
	count := 0
	byteFallback := proc.model.GetTrainerSpec().GetByteFallback()
	for i := 0; i >= 0; i = symList[i].next {
		symbol := symList[i].symbol
		id := proc.symbolToID(symbol)
		if id == proc.unknownID && byteFallback {
			for j := 0; j < len(symbol); j++ {
				if emit != nil {
					emit(proc.byte2ID[symbol[j]])
				}
				count++
			}
			continue
	...
	scratch.syms = symList
	scratch.queue = mergeQueue.Backing()
	scratch.buf = buf
	return count
}

func (proc *Processor) getScratch() *encodeScratch {
	return proc.scratchPool.Get().(*encodeScratch)
}

func (proc *Processor) putScratch(s *encodeScratch) {
	clear(s.syms)
	if cap(s.syms) > 65536 {
		s.syms = nil
	} else {
	...
	if cap(s.queue) > 65536 {
		s.queue = nil
	} else {
		s.queue = s.queue[:0]
	}
	if cap(s.normalized) > 65536 {
		s.normalized = nil
	} else {
		s.normalized = s.normalized[:0]
	}
	if cap(s.buf) > proc.maxPieceLength*4 {
		s.buf = make([]byte, proc.maxPieceLength)
	}
	proc.scratchPool.Put(s)
}
