...
func TestCacheAlignerAlignPrompt(t *testing.T) {
	tests := []struct {
		name  string
		count int
		block int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewCacheAligner(fixedCountEngine(tt.count)).AlignPrompt("ignored", tt.block)
			assertCacheReport(t, got, tt.want)
		})
	}
}

func TestCacheAlignerProfileMinimum(t *testing.T) {
	report := NewCacheAligner(fixedCountEngine(900)).AlignPromptToProfile("ignored", CacheProfile{
		Name:          "test",
		BlockSize:     128,
		MinimumTokens: 1024,
	})
	...
	assertCacheReport(t, report, want)
}

func TestCacheAlignerInvalidConfig(t *testing.T) {
	if got := NewCacheAligner(nil).AlignPrompt("x", 128); got.StrategyHint == "" {
		t.Fatal("nil engine returned empty strategy hint")
	}
	if got := NewCacheAligner(fixedCountEngine(10)).AlignPrompt("x", 0); got.StrategyHint == "" {
		t.Fatal("invalid block returned empty strategy hint")
	}
}

func TestCacheProfileOpenAI(t *testing.T) {
	report := NewCacheAligner(fixedCountEngine(1025)).AlignPromptToProfile("ignored", CacheProfileOpenAI)
...
func assertCacheReport(t *testing.T, got CacheReport, want CacheReport) {
	t.Helper()
	if got.CurrentTokens != want.CurrentTokens ||
		...
		t.Fatalf("CacheReport = %+v, want %+v", got, want)
	}
	if got.StrategyHint == "" {
